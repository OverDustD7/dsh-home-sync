...
源码仓库中的[原审计报告](DSH插件检查与修改报告.md)保留 0.1.0 的问题与证据；[实际修改说明](DSH插件修复完成报告.md)记录 0.2.0 的修复与验证边界。审计报告不包含在运行安装包内。

## 历史复核记录（2026-09-07，已被后续复审补充）

**以下是 0.2.0 的历史记录，其“未发现新的高风险缺陷”结论已被后续复审推翻。** 当前修复和验证以源码仓库中的 [0.2.1 二次审查与修改报告](DSH插件二次审查报告.md) 为准；默认隐藏即时日志的交互保持，新增独立折叠操作记录入口。
